...
select count(*) as female ,(select count(*) from employees where gender = 'M') as male
from employees where gender = 'F';

-- 문제4.
-- 현재 근무하고 있는 직원 수는 몇 명입니까? (salaries 테이블을 사용합니다.) 

select count(distinct emp_no) as '현재 근무하는 직원수' from salaries;
...
select count(distinct emp_no) as '현재 부서 매니저 수' from dept_manager;
...
select count(*) as '사원 수' from salaries where salary > 120000 and current_date < to_date;
...
select count(*) as 'current Engineer employees' from titles where title = 'Engineer' and to_date > current_date;

-- 문제11
-- 사번이 13250(Zeydy)인 직원이 직책 변경 상황을 시간순으로 출력해보세요.

select * from titles where emp_no = 13250 order by from_date,to_date;
